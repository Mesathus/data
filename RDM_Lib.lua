...
\cs(255, 255, 64)${key_bind_savage} \cs(200, 200, 200)Bow Lock:\cr \cs(125,125,255)${player_current_savageMode|Normal}
]]
...
keybinds_on['key_bind_savageMode'] = '       '
...
function validateTextInformation()
	...
	main_text_hub.player_current_savageMode = savageMode.current
...
function self_command(command)
    ...
    if #commandArgs:split(' ') >= 2 then
        ...
        if commandArgs[1] == 'toggle' then
			...
			elseif commandArgs[2] == 'savageMode' then
				savageModes:cycle()
				check_range_lock()
				if use_UI == true then                    
                    validateTextInformation()
                else
                    windower.add_to_chat(8,"----- Savage Lock Mode is now: "..tostring(savageMode.current)) 
				end
            end
        end
